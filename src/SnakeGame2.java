...
public class SnakeGame2 extends GameBase{
	...
	SnakeNN2[] neuNet = new SnakeNN2[numOfGames];
	SnakeNN2 smartestSnake;
	int[] nnLayers = {400, 100, 24, 4};
	...
	private File snakeNNFile = new File("BestSnakeNN2.txt");
	...
	public void initialize() {
		...
		for(int i = 0; i < numOfGames; i++) {
			...
			neuNet[i] = new SnakeNN2(nnLayers, snake[i], apple[i]);
		...
		if(snakeNNFile != null) {
			for(int i = 0; i < numOfGames/2; i++) {
				neuNet[i] = new SnakeNN2(nnLayers, snake[i], apple[i], snakeNNFile);
			}
		}
...
//			if(pressed[RT]) snake.changeDirection(SnakePlayer.RT);
		...
		if(!isInPlayback) {
			...
			if(allDead) {
				...
				if(snakeNNFile == null) {
					File file = new File("BestSnakeNN2.txt");
					snakeNNFile = file;
				}
